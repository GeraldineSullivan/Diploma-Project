...
# Reworked jumping for more responsive control for a less floaty feel
# Overhauled wall jump logic. The player now jumps off the wall instead of climbing it
# Added player bounce mechanic to bounce off enemies/spikes
# Implemented visual feedback, ie: player flashes red when damaged
# Added enemy touch damage for blob enemies
# Introduced flying enemies (bats) and a "boss" version (shard bats)
# Created permanent health and skill (wall jump) upgrades
# Added persistence: doors and switches remain active after respawn
# Ensured collected coins stay collected after death, so coins remain limited
# Designed a boss room system for shard bats - doors close when trap triggers, open after victory

# And more fixes and additional scripts. 


#TO DO NEXT:
# Fix the shard bat area so that player cannot accidentally kill a shard bat before closing the door - GAME BREAKING
# Add new hazards
# Add save points
